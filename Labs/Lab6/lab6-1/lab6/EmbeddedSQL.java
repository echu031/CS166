...
public class EmbeddedSQL {
   ...
   public static void Query1(EmbeddedSQL esql){
      try {
	String select = "SELECT S.sid, COUNT(C.pid) AS TotalParts ";
	String from = "FROM Catalog C, Suppliers S ";
	String where = "WHERE S.sid=C.sid ";
	String group = "GROUP BY S.sid";
   ...
   public static void Query2(EmbeddedSQL esql){
	...
	String select = "SELECT S.sid, COUNT(C.pid) AS TotalParts ";
        ...
        String where = "WHERE S.sid=C.sid ";
        String group = "GROUP BY S.sid ";
	String having = "HAVING COUNT(C.pid) > 2";
	String query = select + from + where + group + having;
        int rowCount = esql.executeQuery(query);
        System.out.println("total row(s): " + rowCount);
      } catch(Exception e) {
          System.err.println(e.getMessage());
   ...
   public static void Query3(EmbeddedSQL esql){
      try {
	String select = "SELECT S.sname, COUNT(C.pid) AS TotalParts ";
        String from = "FROM Catalog C, Parts P, Suppliers S ";
        String where = "WHERE S.sid=C.sid AND P.pid=C.pid ";
	String group = "GROUP BY S.sid, S.sname ";
	String having = "HAVING EVERY (P.color='Green')";
	String query = select + from + where + group + having;
	int rowCount = esql.executeQuery(query);
        System.out.println("total row(s): " + rowCount);
      } catch(Exception e) {
   ...
   public static void Query4(EmbeddedSQL esql){
      try {
	String select = "SELECT S.sname, MAX(C.cost) AS PartPrice ";
	String from = "FROM Catalog C, Suppliers S ";
	String where = "WHERE S.sid=C.sid AND C.sid IN ";
	String nestedSelect = "(SELECT C2.sid ";
	String nestedFrom = "FROM Catalog C2, Parts P2 ";
	String nestedWhere = "WHERE P2.pid=C2.pid AND P2.color='Red' ";
	String intersect = "INTERSECT ";
	String interSelect = "SELECT C3.sid ";
	String interFrom = "FROM Catalog C3, Parts P3 ";
	String interWhere = "WHERE P3.pid=C3.pid AND P3.color='Green') ";
	String group = "GROUP BY S.sid, S.sname";
	String query = select + from + where + nestedSelect + nestedFrom + nestedWhere + intersect + interSelect + interFrom + interWhere + group;
	int rowCount = esql.executeQuery(query);
	System.out.println("total row(s): " + rowCount);
      } catch(Exception e) {
	  System.err.println(e.getMessage());
   ...
   public static void Query5(EmbeddedSQL esql){
	...
	System.out.print("\tEnter cost: ");
	String input = in.readLine();
	String select = "SELECT DISTINCT P.pname ";
	String from = "FROM Catalog C, Parts P ";
	String where = "WHERE C.cost<" + input + " AND C.pid=P.pid";
   ...
   public static void Query6(EmbeddedSQL esql){
	...
	String input = in.readLine();
	String select = "SELECT DISTINCT S.address ";
	String from = "FROM Catalog C, Parts P, Suppliers S ";
	String where = "WHERE P.pname='" + input + "' AND P.pid=C.pid AND C.sid=S.sid";
	String query = select + from + where;
	int rowCount = esql.executeQuery(query);
	System.out.println("total row(s): " + rowCount);
      } catch(Exception e) {
